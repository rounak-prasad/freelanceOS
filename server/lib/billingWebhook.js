 ...
 */
export async function applyBillingWebhook(db, body) {
  const event = body?.event;
  const entity = body?.payload?.subscription?.entity || {};
  const providerSubId = entity.id;
  let wsId = entity.notes?.workspaceId || null;
  if (!wsId && providerSubId) wsId = (await repo.findSubscriptionByProviderId(db, providerSubId))?.workspace_id || null;

  await repo.insertBillingEvent(db, { workspaceId: wsId, provider: 'razorpay', eventType: event || 'unknown', payload: body?.payload || null });
  ...
  if (wsId && status) {
    ...
    return { wsId, status, applied: await repo.upsertSubscription(db, wsId, patch) };
  }
