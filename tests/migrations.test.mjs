...
const db = await createMemoryDb(); // createMemoryDb() already runs migrations on boot

const applied = (await db.all('SELECT id FROM schema_migrations ORDER BY id')).map((r) => r.id);
...
for (const t of ['users', 'workspaces', 'memberships', 'clients', 'invoices', 'invitations', 'subscriptions', 'billing_events', 'email_tokens', 'refresh_tokens', 'audit_log']) {
  const row = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name = ?", [t]);
